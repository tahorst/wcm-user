...
def main(seedOutDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata = None):
	...
	for simDir in allDirs:
		...
		totalTime = np.append(totalTime, time)
		...
		if tfBoundCounts == []:
			...
			tfBoundCounts = np.concatenate((tfBoundCounts, bulkMoleculesReader.readColumn("counts")[:, tfBoundIndex]))
		...
		if synthProbs == []:
			...
			synthProbs = np.concatenate((synthProbs, rnaSynthProbReader.readColumn("rnaSynthProb")[:, synthProbIndex]))
		...
		ax.plot(time, trpConcentration.asNumber(units.umol / units.L), color = "b", linewidth = 2)
		...
		# # ax.plot(time, trpRInactiveCounts, color = "g")
		# # ax.plot(time, trpRTotalCounts, color = "r")
		# ax.plot(time, trpRActiveCounts / trpRTotalCounts, color = "b")
		# plt.ylabel("Fraction TrpR Active", fontsize = 6)
		...
		ax.plot(time, trpAProteinTotalCounts, color = "b", linewidth = 2)
		...
		ax.plot(time, trpAProteinTotalConcentration.asNumber(units.umol / units.L), color = "b", linewidth = 2)
		plt.ylabel("TrpA Concentration [uM]", fontsize = 6)
		...
		ax.plot(time, cellMass.asNumber(units.fg), color = "b", linewidth = 2)
		...
		ax.set_xticklabels(["%0.0f" % xmin, "%0.0f" % xmax])
		ax.spines['top'].set_visible(False)
		# ax.spines['bottom'].set_visible(False)
		ax.spines['right'].set_visible(False)
		# ax.xaxis.set_ticks_position('none')
	...
	##############################################################
	ax = plt.subplot(nRows, 1, 2)
	ax.plot(totalTime[width:-width], tfBoundCountsMA[width:-width, 0], linewidth = 2)
	...
	ax.plot(totalTime[width:-width], synthProbsMA[width:-width, 0], linewidth = 2)
