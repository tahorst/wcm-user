...
COMPRESS_OUTPUT = str(os.environ.get("COMPRESS_OUTPUT", "0"))
	...
	for j in xrange(N_INIT_SIMS):
		...
		for k in xrange(N_GENS):
			...
			for l in (xrange(2**k) if not SINGLE_DAUGHTERS else [0]):
				...
				wf_links[fw_this_variant_this_gen_this_sim].append(fw_this_variant_sim_data_compression)
				wf_links[fw_this_variant_this_gen_this_sim].append(fw_validation_data_compression)
				wf_links[fw_this_variant_this_gen_this_sim].append(fw_this_variant_this_gen_this_sim_compression)
