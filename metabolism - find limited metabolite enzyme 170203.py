...
class Metabolism(wholecell.processes.process.Process):
	...
	def evolveState(self):
		...
		if self._sim.time() > 26200 and deltaMetabolites[self.metaboliteNames.index("GLUTATHIONE[c]")] <= 0:
			for cidx in range(len(self.catalystsList)):
				...
				idx = np.where(kineticsEnzymesCountsInit == 0)

				catalystsCountsInit[cidx] += 1
				if self.catalystsList[cidx] in self.kineticsEnzymesList:
					eidx = self.kineticsEnzymesList.index(self.catalystsList[cidx])
					kineticsEnzymesConcentrations[eidx] = countsToMolar * (kineticsEnzymesCountsInit[eidx] + 1)
				...
				if deltaMetabolites[self.metaboliteNames.index("GLUTATHIONE[c]")] > 0:
					print self.catalystsList[cidx]
